...
public class PatientServiceImpl implements PatientService {
	...
	public Patient updatePatient(Long id, Patient p) {
		...
		if (p.getEmail() != null)
			existing.setEmail(p.getEmail().trim());
		if (p.getPhone() != null)
			existing.setPhone(p.getPhone().trim());
	...
	public List<PatientDto> searchPatients(String query) {
		return repo.searchPatients(query).stream().map(PatientMapper::toDto).collect(Collectors.toList());
	}
}
